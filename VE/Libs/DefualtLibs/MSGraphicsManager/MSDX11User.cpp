...
void* initDX11(void* hwnd)
{
	...
	struct DX11Devices* rs = (DX11Devices*)malloc(sizeof(DX11Devices));

	HRESULT hr = { 0 };
	for (unsigned char index = 0; index < ARRAYSIZE(drivers);)
	{
		hr = D3D11CreateDevice(nullptr, drivers[index], 0, 0, 0, 0, D3D11_SDK_VERSION, &rs->dev, 0, &rs->context);
...
__declspec(noinline)
void DX11_createSwapChain(DX11Devices* rs, HWND hwnd)
{
	...
	sw_desc.Flags = DXGI_SWAP_CHAIN_FLAG::DXGI_SWAP_CHAIN_FLAG_ALLOW_MODE_SWITCH;
	//How to format the color channels of each the pixel 
	sw_desc.BufferDesc.Format = DXGI_FORMAT::DXGI_FORMAT_R8G8B8A8_UNORM;
	//Setting RefreshRate
	sw_desc.BufferDesc.RefreshRate = { 10U, 0U };
	//How well to sample colors from a texture on to each pixel
	sw_desc.SampleDesc = { 1U, 0U };
	...
	sw_desc.SwapEffect = DXGI_SWAP_EFFECT::DXGI_SWAP_EFFECT_DISCARD;
